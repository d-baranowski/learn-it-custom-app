...
func NewConnectionManager(url string, conf amqp.Config, log *zap.Logger, reconnectInterval time.Duration) (*ConnectionManager, error) {
...
func (connManager *ConnectionManager) Close() error {
	connManager.logger.Info("closing connection manager...")
...
func (connManager *ConnectionManager) startNotifyClose() {
	...
	err := <-notifyCloseChan
	if err != nil {
		connManager.logger.Error("attempting to reconnect to amqp server after connection close with error", zap.Error(err))
		connManager.reconnectLoop()
		connManager.logger.Warn("successfully reconnected to amqp server")
		connManager.dispatcher.Dispatch(err)
	}
	if err == nil {
		connManager.logger.Info("amqp connection closed gracefully")
	}
}
...
func (connManager *ConnectionManager) reconnectLoop() {
	for {
		connManager.logger.Info("waiting to attempt to reconnect to amqp server", zap.Duration("reconnect_interval", connManager.ReconnectInterval))
		time.Sleep(connManager.ReconnectInterval)
		err := connManager.reconnect()
		if err != nil {
			connManager.logger.Error("error reconnecting to amqp server", zap.Error(err))
		} else {
...
func (connManager *ConnectionManager) reconnect() error {
	...
	if err = connManager.connection.Close(); err != nil {
		connManager.logger.Warn("error closing connection while reconnecting", zap.Error(err))
	}
