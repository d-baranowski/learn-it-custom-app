...
// NewCPPublisher returns a new publisher connected to the given rabbitmq server using the connection pool.
func NewCPPublisher(optionFuncs ...func(*PublisherOptions)) (*Publisher, error) {
	...
	return NewPublisher(conn, optionFuncs...)
}
...
func NewPublisher(conn *Connection, optionFuncs ...func(*PublisherOptions)) (*Publisher, error) {
	...
	go func() {
		for err := range publisher.reconnectErrCh {
			publisher.options.Logger.Info("successful publisher recovery from error", zap.Error(err))
			err := publisher.startup()
			if err != nil {
				publisher.options.Logger.Fatal("error on startup for publisher after cancel or close", zap.Error(err))
...
func (publisher *Publisher) Close() {
	...
	if err != nil {
		publisher.options.Logger.Warn("error while closing the channel", zap.Error(err))
	}

	//publisher.isClosedMux.Lock()
	//defer publisher.isClosedMux.Unlock()
	//publisher.isClosed = true

	publisher.options.Logger.Debug("closing publisher...")
	go func() {
		publisher.closeConnectionToManagerCh <- struct{}{}
	}()

	connectionPool.ReleaseID(publisher.connID)
}

func (publisher *Publisher) IsClosed() bool {
	//publisher.isClosedMux.RLock()
	//defer publisher.isClosedMux.RUnlock()
