package rabbitmq

import (
	amqp "github.com/rabbitmq/amqp091-go"
	"github.com/segmentio/ksuid"
	"go.uber.org/zap"
	"pkg/rabbitmq/internal/connectionmanager"
)

// Connection manages the connection to a rabbit cluster
// it is intended to be shared across publishers and consumers
type Connection struct {
	id                         string
	connectionManager          *connectionmanager.ConnectionManager
	reconnectErrCh             <-chan error
	closeConnectionToManagerCh chan<- struct{}

	options ConnectionOptions
}

// AmqpConfig wraps amqp.Config
// AmqpConfig is used in DialConfig and Open to specify the desired tuning
// parameters used during a connection open handshake.  The negotiated tuning
// will be stored in the returned connection's AmqpConfig field.
type AmqpConfig amqp.Config

// NewConnection creates a new connection manager
func NewConnection(url string, optionFuncs ...func(*ConnectionOptions)) (*Connection, error) {
	defaultOptions := getDefaultConnectionOptions()
	options := &defaultOptions
	for _, optionFunc := range optionFuncs {
		optionFunc(options)
	}

	manager, err := connectionmanager.NewConnectionManager(url, amqp.Config(options.Config), options.Logger, options.ReconnectInterval)
	if err != nil {
		return nil, err
	}

	reconnectErrCh, closeCh := manager.NotifyReconnect()
	conn := &Connection{
		id:                         ksuid.New().String(),
		connectionManager:          manager,
		reconnectErrCh:             reconnectErrCh,
		closeConnectionToManagerCh: closeCh,
		options:                    *options,
	}

	go conn.handleRestarts()
	return conn, nil
}

func (conn *Connection) handleRestarts() {
	for err := range conn.reconnectErrCh {
		conn.options.Logger.Info("successful connection recovery from error", zap.Error(err))
	}
}

func (conn *Connection) Manager() *connectionmanager.ConnectionManager {
	return conn.connectionManager
}

// Close closes the connection, it's not safe for re-use.
// You should also close any consumers and publishers before
// closing the connection
func (conn *Connection) Close() error {
	conn.closeConnectionToManagerCh <- struct{}{}
	return conn.connectionManager.Close()
}
