...
func New(key []byte, previousKey []byte) (sqlcrypt.Crypterer, error) {
	if len(key) != 32 {
...
func (a *AESCrypter) Encrypt(w io.Writer, r io.Reader) error {
	...
	nonce, err := sqlcrypt.GenerateBytes(a.current.NonceSize())
	if err != nil {
...
var _ sqlcrypt.Crypterer = (*AESCrypter)(nil)
