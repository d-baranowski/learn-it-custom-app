...
// A IChoice contains a generic item and a weight controlling the frequency with
...
// IWeightedChoice used weighted random selection to return one of the supplied
...
// choice weighted 1.
func IWeightedChoice(choices []IChoice) (IChoice, error) {
...
// returned equally often; and each will be returned 3 times as often as a
// choice weighted 1.
func WeightedChoice[T any](choices []Choice[T]) (Choice[T], error) {
	// Based on this algorithm:
	//     http://eli.thegreenplace.net/2010/01/22/weighted-random-generation-in-python/
	var ret Choice[T]
	sum := 0
