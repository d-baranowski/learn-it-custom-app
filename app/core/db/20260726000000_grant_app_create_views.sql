-- Grant the `app` login role CREATE on the core schema so it can materialise the
...
--
--   CREATE VIEW core.therapist_e8e380c35625_en
--     WITH (security_barrier = true, security_invoker = true) AS SELECT ...
--
-- so every grid failed with:
--
--   ERROR: permission denied for schema core (SQLSTATE 42501)
--
-- Note that message is identical for missing USAGE and missing CREATE, which
-- makes this easy to misdiagnose: `has_schema_privilege(app, core, 'USAGE')`
...
BEGIN
    ...
    EXECUTE 'GRANT CREATE ON SCHEMA core TO app';
END
$$;
-- +goose StatementEnd
...
BEGIN
    ...
    EXECUTE 'REVOKE CREATE ON SCHEMA core FROM app';
END
$$;
-- +goose StatementEnd
