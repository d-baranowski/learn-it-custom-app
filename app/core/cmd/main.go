...
func main() {
	...
	// go run main.go -migrate=true or MIGRATE=true go run main.go
	...
	if !shouldMigrate {
		envMigrate := strings.ToLower(os.Getenv("MIGRATE"))
		shouldMigrate = envMigrate == "true" || envMigrate == "1" || envMigrate == "yes"
	}

	if shouldMigrate {
		zap.L().Info("starting core migrations")
		migrator := fx.New(
			fx.Provide(
