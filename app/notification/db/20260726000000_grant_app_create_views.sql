-- Grant the `app` login role CREATE on the notification schema so it can materialise the
...
--
--   CREATE VIEW notification.<entity>_<hash>_en
--     WITH (security_barrier = true, security_invoker = true) AS SELECT ...
--
-- so every grid failed with:
--
--   ERROR: permission denied for schema notification (SQLSTATE 42501)
--
-- Note that message is identical for missing USAGE and missing CREATE, which
-- makes this easy to misdiagnose: `has_schema_privilege(app, notification, 'USAGE')`
...
BEGIN
    ...
    EXECUTE 'GRANT CREATE ON SCHEMA notification TO app';
END
$$;
-- +goose StatementEnd
...
BEGIN
    ...
    EXECUTE 'REVOKE CREATE ON SCHEMA notification FROM app';
END
$$;
-- +goose StatementEnd
